...
import { z } from 'zod';
import { ENVIRONMENTS, config, environmentForMode } from '../config.ts';
import { getActiveEnvironment, getSymbolFilters, setActiveEnvironment } from '../binance/rest.ts';
import { settingsUpdateSchema } from '../services/settingsService.ts';
  ...
  router.get('/settings', (_request, response) => {
    response.json({
      ...
      binance: {
        ...
        // a interface só sabe SE existe credencial; a chave nunca sai do servidor
        production: { credentialsConfigured: ENVIRONMENTS.production.hasCredentials },
        testnet: { credentialsConfigured: ENVIRONMENTS.testnet.hasCredentials },
      },
